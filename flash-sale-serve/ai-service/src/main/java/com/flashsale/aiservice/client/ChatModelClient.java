...
public class ChatModelClient {
  ...
  public String chat(String systemPrompt, String userPrompt) {
    ...
    try {
      log.info("Invoking chat model, model={}, systemPromptChars={}, userPromptChars={}",
        aiProperties.getChatModel(), systemPrompt.length(), userPrompt.length());

      ChatResponse response = aiWebClient.post()
        .uri("/v1/chat/completions")
        .bodyValue(new ChatRequest(aiProperties.getChatModel(), List.of(
